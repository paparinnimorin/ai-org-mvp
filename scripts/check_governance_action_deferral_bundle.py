...
def main() -> int:
    p = argparse.ArgumentParser(description="Check governance action deferral bundle.")
    ...
    p.add_argument("--process", default="docs/process/governance-action-deferral.md")
    p.add_argument("--checklist", default="docs/checklists/governance-action-deferral-checklist.md")
    ...
    if deferral_report and deferral_report.exists():
        print(f"[OK] deferral report found: {deferral_report}")
    else:
        print(f"[NG] deferral report missing for minutes {args.minutes} in {deferrals_dir}")
        ok = False

    if process_file.exists():
        print(f"[OK] deferral process found: {process_file}")
    else:
        print(f"[NG] deferral process missing: {process_file}")
        ok = False

    if checklist_file.exists():
        print(f"[OK] deferral checklist found: {checklist_file}")
    else:
        print(f"[NG] deferral checklist missing: {checklist_file}")
        ok = False
