...
## Todo
- [ ] Task & Calendarの統合手順を定義（会議調整/リマインド/優先順位）
- [ ] Comms分類ルールを定義（即時対応/要承認/保留/スパム）
- [ ] Data運用の最低基準を定義（毎日バックアップ、30日世代管理）
- [ ] セキュリティ対応の責務分界を定義（Ops一次対応、Tech二次対応）
- [ ] Ops例外対応フローを作成（障害・誤送信・未返信滞留）
- [ ] Human向け週次サマリー様式を作成（節約時間/自動化率/残課題）
